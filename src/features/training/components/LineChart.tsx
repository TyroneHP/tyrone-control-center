...
export function LineChart({ ariaLabel, data, onSelect, unit }: LineChartProps) {
  const titleId = useId()
  ...
  const minimum = Math.min(...visual.map(({ value }) => value))
  const maximum = Math.max(...visual.map(({ value }) => value))
